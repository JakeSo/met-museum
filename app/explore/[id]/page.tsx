  ...
  return (
    <div className="w-full space-y-10 pb-16">
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_380px]">
        {/* Images */}
        {allImages.length > 0 && <div className="space-y-4">
          {allImages[0] && (
        ...
        <aside className="space-y-6 lg:sticky lg:top-6 lg:self-start max-w-200">
          <div className="space-y-1">
            {artwork.isHighlight && <Badge variant="secondary" className="mb-2">Featured</Badge>}
            <h1 className="text-2xl font-semibold leading-snug">{artwork.title}</h1>
            {artwork.artistDisplayName && (
              <p className="text-base text-muted-foreground">by {artwork.artistDisplayName}</p>
            )}
            {artistLine && (
          ...
          <dl className="space-y-3">
            ...
            <Field label="Gallery" value={artwork.GalleryNumber ? `Gallery ${artwork.GalleryNumber}` : ''} />
            <Field label="Credit" value={artwork.creditLine} />
            <Field label="Accession" value={artwork.accessionNumber} />
            <Field
              label="Rights"
              value={artwork.isPublicDomain ? 'Public domain' : artwork.rightsAndReproduction}
            />
          </dl>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {artwork.tags.map(tag => (
                  <Badge key={tag.term} variant="outline">{tag.term}</Badge>
                ))}
              </div>
            </div>
          )}
