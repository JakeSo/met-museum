...
export default function SearchBar() {
  ...
  return (
    <form onSubmit={submit} className="w-full max-w-150 mx-auto space-y-3 border-b-2 pb-6">
      <InputGroup>
      ...
      <div className="flex flex-col gap-2">
        <div role="group" aria-labelledby="search-scope-label" className="flex flex-wrap items-center gap-1.5">
          ...
          {SCOPE_CHIPS.map(({ key, label }) => (
            <SearchChip key={key} active={!!scope[key]} onClick={() => toggleFlag(scope, setScope, key)}>
              {label}
            </SearchChip>
          ))}
        </div>
        ...
        <div role="group" aria-labelledby="search-filter-label" className="flex flex-wrap items-center gap-1.5">
          ...
          {FILTER_CHIPS.map(({ key, label }) => (
            <SearchChip key={key} active={!!filterFlags[key]} onClick={() => toggleFlag(filterFlags, setFilterFlags, key)}>
              {label}
            </SearchChip>
          ))}
          <SearchChip active={showAdvanced} onClick={() => setShowAdvanced(v => !v)} className="ml-auto">
      ...
      {showAdvanced && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <SearchField label="Medium" hint="comma-separated" htmlFor="search-medium">
            <Input
              id="search-medium"
          ...
          <SearchField label="Geographic location" hint="comma-separated" htmlFor="search-geo-location">
            <Input
