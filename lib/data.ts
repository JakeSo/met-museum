...
export const search = async (
  ...
  if (options) {
    ...
    if (isHighlight !== undefined)
      url.searchParams.set("isHighlight", String(isHighlight));
    ...
    if (isOnView !== undefined)
      url.searchParams.set("isOnView", String(isOnView));
    if (artistOrCulture !== undefined)
      url.searchParams.set("artistOrCulture", String(artistOrCulture));
    if (medium?.length)
      url.searchParams.set("medium", String(medium.join("|")));
    if (hasImages !== undefined)
      url.searchParams.set("hasImages", String(hasImages));
    if (geoLocation?.length)
      url.searchParams.set("geoLocation", geoLocation.join("|"));
...
export const fetchObject = unstable_cache(
  async (objectId: number): Promise<MuseumObject> => {
    ...
    if (!response.ok)
      throw new Error(
        `Failed to fetch object ${objectId}: ${response.statusText}`,
      );
    return response.json();
  },
  ["fetchObject"],
